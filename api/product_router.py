...
from api.rate_limiter import check_demo_limit
...
async def update_product_stock(product_id: str, request: UpdateStockRequestModel, use_case: UpdateStock = Depends(get_update_stock_use_case), _: None = Depends(check_demo_limit)) -> UpdateStockResponseModel:
    try:
...
async def create_product(request: CreateProductRequest, tenant_id: str = Header(..., alias='X-Tenant-ID'), use_case: CreateProduct = Depends(get_create_use_case), _: None = Depends(check_demo_limit)):
    try:
